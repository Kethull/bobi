...
class DetailedProbeRenderer:
    ...
    def _draw_main_hull(self, surface, pos, angle, energy_ratio, scale):
        ...
        hull_size = config.Visualization.PROBE_SIZE_SIM * scale
    ...
    def _draw_communication_dish(self, surface, pos, angle, scale):
        ...
        dish_base_size = config.Visualization.PROBE_SIZE_SIM * 1.1 * scale # Slightly smaller than prompt for balance
        ...
        dish_offset_local = np.array([config.Visualization.PROBE_SIZE_SIM * 0.5 * scale, 0])
        ...
        for i in range(3): # Three supports
            ...
            hull_attach_local = np.array([config.Visualization.PROBE_SIZE_SIM * 0.1 * scale,
                                          (i-1) * config.Visualization.PROBE_SIZE_SIM * 0.2 * scale]) # Spread along y-axis of hull
    ...
    def _draw_solar_panels(self, surface, pos, angle, energy_ratio, scale):
        ...
        panel_length = config.Visualization.PROBE_SIZE_SIM * 1.6 * scale # Slightly shorter
        panel_width = config.Visualization.PROBE_SIZE_SIM * 0.5 * scale  # Slightly wider
        ...
        for side in [-1, 1]: # Left and right panels
            ...
            mount_x_local = -config.Visualization.PROBE_SIZE_SIM * 0.2 * scale
            
            # Panel corners relative to a central line extending sideways from mount_x_local
            # Start of panel (closer to hull)
            start_y_local = side * (config.Visualization.PROBE_SIZE_SIM * 0.3 * scale) # Gap from hull center
            end_y_local = side * (config.Visualization.PROBE_SIZE_SIM * 0.3 * scale + panel_length)
    ...
    def _draw_sensor_arrays(self, surface, pos, angle, scale):
        ...
        fwd_sensor_offset = np.array([config.Visualization.PROBE_SIZE_SIM * 0.9 * scale, 0]) # Further forward
        ...
        for side in [-1, 1]:
            side_sensor_offset = np.array([config.Visualization.PROBE_SIZE_SIM * 0.2 * scale, side * config.Visualization.PROBE_SIZE_SIM * 0.4 * scale])
    ...
    def _draw_rcs_thrusters(self, surface, pos, angle, scale):
        ...
        rcs_local_positions = [
            np.array([ config.Visualization.PROBE_SIZE_SIM*0.6*scale,  config.Visualization.PROBE_SIZE_SIM*0.3*scale]), # Front-ish right
            np.array([ config.Visualization.PROBE_SIZE_SIM*0.6*scale, -config.Visualization.PROBE_SIZE_SIM*0.3*scale]), # Front-ish left
            np.array([-config.Visualization.PROBE_SIZE_SIM*0.5*scale,  config.Visualization.PROBE_SIZE_SIM*0.4*scale]), # Rear-ish right
            np.array([-config.Visualization.PROBE_SIZE_SIM*0.5*scale, -config.Visualization.PROBE_SIZE_SIM*0.4*scale]), # Rear-ish left
        ]
    ...
    def _draw_status_indicators(self, surface, pos, angle, energy_ratio, is_low_power, scale):
        ...
        indicator_local_positions = [
            np.array([config.Visualization.PROBE_SIZE_SIM * 0.7 * scale, 0]),                         # Front center
            np.array([-config.Visualization.PROBE_SIZE_SIM * 0.4 * scale,  config.Visualization.PROBE_SIZE_SIM * 0.35 * scale]), # Rear right
            np.array([-config.Visualization.PROBE_SIZE_SIM * 0.4 * scale, -config.Visualization.PROBE_SIZE_SIM * 0.35 * scale]), # Rear left
        ]
